...
@Controller
public class ReviewBoardListHandler implements CommandHandler{
	
	@Resource
	private ShopDAO shopdao;
	
	@RequestMapping("/reviewBoardList") // .do 생략 가능
	@Override
	public ModelAndView process(HttpServletRequest request, HttpServletResponse response) throws Throwable {
		...
		if(count != 0) {
			...
			map.put("start", start);
			map.put("end", end);
			List<BoardDataBean> dtos = shopdao.getArticles(map); // board.xml에 넘길 수 있는 파라미터가 하나 뿐이므로 Map으로 start와 end를 넘긴다.
			request.setAttribute("dtos", dtos);
		}
		
		return new ModelAndView("/reviewBoard/reviewBoardList");
	}
}
